...
class CompatibilityTest extends TestSpec {
  ...
  it should "deserialize a name change: V2(title, year, director) to V3(title, released_year, director) " in {
    ...
    val bytes: Array[Byte] = BinarySerializer.serializeV2(obj)

    val in = new SeekableByteArrayInput(bytes)

    val result = deserialize[MovieChangedV3](SchemaRegistry.movieChanged(2), SchemaRegistry.movieChanged(3), in)
  ...
  it should "deserialize a field drop and a new one of the same tipe : V3(title, released_year, director) to V4(title, director, wonOscars)" in {
    ...
    val bytes: Array[Byte] = BinarySerializer.serializeV3(obj)

    val in = new SeekableByteArrayInput(bytes)

    val result = deserialize[MovieChangedV4](SchemaRegistry.movieChanged(3), SchemaRegistry.movieChanged(4), in)
  ...
  it should "deserialize adding a complex parameter: V4(title, director, wonOscars) to V4(title, director, wonOscars, releases)" in {
    ...
    val bytes: Array[Byte] = BinarySerializer.serializeV4(obj)

    val in = new SeekableByteArrayInput(bytes)

    val result = deserialize[MovieChangedV5](SchemaRegistry.movieChanged(4), SchemaRegistry.movieChanged(5), in)
